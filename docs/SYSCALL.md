# Zuki System Call Interface — Implementer Specification v1.0
...
- calling convention and argument model
...
The kernel does not expose high-level service semantics such as:
...
Unknown syscall numbers must fail with the canonical invalid-syscall error.
...
| `sys_protect` | `0x12` | change mapping permissions within allowed authority |
...
`sys_alloc_frame` is not part of the canonical ABI unless the frame-allocation authority model is explicitly defined as a kernel surface. Otherwise, frame allocation belongs to the authority graph established through init/service construction.
...
| Name | Number | Description |
|---|---:|---|
| `sys_sleep` | `0x30` | block current thread for a bounded duration |
...
### 2.6 Process and capability-space syscalls

| Name | Number | Description |
|---|---:|---|
| `sys_process_create` | `0x50` | create a new process or equivalent authority domain |
| `sys_process_exit` | `0x51` | terminate current process |
| `sys_cap_transfer` | `0x52` | transfer capability through an explicit destination authority |
| `sys_cnode_create` | `0x53` | create a new CNode if supported by the authority model |

Requirements:

- semantics must remain consistent with `INIT.md` and the explicit service graph
- no implicit process privilege
...
This document is the ground truth for syscall semantics in Zuki.
